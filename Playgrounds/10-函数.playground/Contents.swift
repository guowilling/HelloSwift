...
// 寻找数组中的最大值和最小值
func minMax(array: [Int]) -> (min: Int, max: Int)? {   // 使用元组返回多个值
    if array.isEmpty {
        return nil
    }
...
let results = minMax(array: [3, 5, 6, 12, 8, 10]) // results 是一个元组类型
print(results!.max)                               // 通过点语法取值
print(results!.min)                               // '!' 强制解包


// 函数的参数标签和参数名称: 参数标签是在函数调用的时候使用, 参数名称是在函数的实现中使用
func someFunction(label name: String) -> String { // label 参数标签, name 参数名称
    return name                                   // 函数实现中使用参数名称
}
someFunction(label: "Dariel")                     // 调用函数的时候使用参数标签
// 参数标签目的: 使函数在调用的时候表达更有力, 并且保持函数内部的可读性
...
defaultLabel(label1: "Lucy")                      // 有默认值参数调用时可以省略
