...
// 访问控制: Swift 中可以对各种类型进行访问控制, 从而隐藏代码的实现细节, 为其他人访问代码提供接口
...
// 访问级别:
// 公开访问: 可以在同一模块中访问任何类型, 导入模块后别的模块也能访问该模块的任何类型
// 内部访问: 只能在模块内部访问, 模块外部不能访问
// 文件私有访问: 只能在文件内部访问
// 私有访问: 只能在被限制的访问作用域内访问


public class PublicClass {}               // 公开访问
internal class InternalClass {}           // 内部访问(如果指明访问限制默认是 internal)
fileprivate class FileprivateClass {}     // 文件私有访问
private class PrivateClass {}             // 私有访问


// 元组类型的访问限制, 由元组中访问级别最严格的类型决定
internal let x = 2
private let y = 3
let tupleA = (x, y) // 此时 tupleA 的访问级别为 private


// 函数的访问限制, 根据最严格的参数类型和返回类型的访问限制来决定
private func someFunction() -> (InternalClass, PrivateClass) {
    return (InternalClass(), PrivateClass()) // 函数前面不加 private, 就无法访问 private 的返回值类型
}
...
//stringOfEdit.numberOfEdits = "XY"  // ❌ numberOfEdits 属性的 set 方法的限制类型为 private, cannot assign to property: 'numberOfEdits' setter is inaccessible
