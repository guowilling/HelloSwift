...
//position2.x = 22.22                 // ❌ 变量属性也不可修改
// 因为结构体属于值类型, 当值类型被声明为常量的时候, 其所有属性都是常量
...
// 类是引用类型, 引用类型赋值给常量的时候, 仍可以修改该实例的变量属性
...
manager.data                       // 此时才会创建 data, 执行耗时的数据加载
...
struct Cuboid {
    ...
    var volume: Double {           // 计算属性必须用 var 修饰, 因为计算属性值是不固定的
