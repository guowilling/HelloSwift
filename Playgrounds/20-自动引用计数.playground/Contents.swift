...
// ARC(Auto Reference Counteting)自动引用计数: Swift 会通过 ARC 来自动管理内存

// 结构体和枚举是值类型, 引用计数只能应用于引用类型->类

// 自动引用计数工作机制: 为了保证实例在使用的过程中不被销毁, ARC会自动计算一个实例被引用的次数, 只要引用次数不等于0, 该实例都不会被销毁
...
var person1: Person?                      // 可选类型的变量方便置为 nil
var person2: Person?
var person3: Person?
...
person3 = nil                             // 最后一个强引用被断开, ARC 会销毁该实例
...
apartment1?.tenant = people1              // 都是强引用
people1 = nil
apartment1 = nil                          // 两个引用都置为 nil 了, 但是实例并没有销毁


// 解决循环引用的三种方式:
// 实例之间的循环强引用的解决办法: 弱引用和无主引用, 对于在生命周期会变为 nil 的实例使用弱引用, 对于初始化后不能被赋值为 nil 的实例使用无主引用
...
otherPeople1 = nil
otherApartment1 = nil                       // 两个实例的 deinit 都会执行
...
class Food {
    let number: Int
    unowned var owner: Dog                  // owner 关键字表示该变量为无主引用
...
// 那么如果两个属性都不能是可选类型呢, 一个使用无主属性, 另一个使用隐式解析可选属性.
class Country {
    let name: String
...
var country: Country? = Country(name: "China", capitalName: "HangZhou") // 这样一条语句就能够创建两个实例
...
class Element {
    ...
    // '() -> String' 相当于一个没有参数, 返回 String 类型的函数
...
var element1: Element? = Element(name: "Hello", text: "Alex")
print(element1!.group())                     // 闭包与实例相互引用
element1 = nil                               // 闭包里的 self 为无主引用, 所以能解除循环引用
