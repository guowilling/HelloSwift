...
// 类型约束: swapTwoValues(_:_:) 和 Stack 适用于任何类型, 但有时我们需要对类型进行一些约束
// 这些约束可以是类型参数必须继承指定类, 或者符合特定的协议或组合协议
...
func findIndex<T: Equatable>(of valueToFind: T, in array:[T]) -> Int? {
    for (index, value) in array.enumerated() {
        if value == valueToFind { // 遵守 Equatable 协议的类型必须实现等式符和不等符号, 才能对两个类型进行比较
            return index
        }
    }
    return nil
