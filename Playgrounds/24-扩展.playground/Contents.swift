...
// swift中扩展的功能: 
// 1.为已有类添加计算型实例属性和计算型类型属性
// 2.为已有类添加便利构造器
// 3.为已有类添加实例方法和类型方法
// 4.为已有类型添加下标
// 5.为已有的类, 结构体, 枚举添加嵌套类型
...
extension Double {
    ...
    //var invalid: Double? // ❌ 编译报错: 扩展不能添加存储属性和属性观察器
}
let oneMeter = 100.cm
...
// 为值类型添加构造器时, 如果该值类型原始实现中未定义任何定制的构造器且所有存储属性提供了默认值, 这时就可以在扩展中的构造器里调用默认构造器和逐一构造器
extension Rect {
