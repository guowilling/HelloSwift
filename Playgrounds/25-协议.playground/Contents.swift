...
// 类, 结构体, 枚举都可以遵守协议, 并为协议的规则提供具体的实现
...
struct SomeStructure: SomeProtocol1 { // 遵守协议 ':' 后面加协议名称, 多个协议之间用逗号隔开
    // 结构体内容...
}
...
protocol SomeProtocol2 {
    ...
    static var someTypeProperty: Int { get set } // 关键字 static 修饰表示类属性
}
...
protocol SomeProtocol3 {
    static func someTypeMethod() // static 修饰表示是类方法
}
...
class RandomNumGenerator: RandomNum {
    func random() -> Int {
        return Int(arc4random() % 10) // 0 ~ 9 的随机数
    }
}
...
class RandomNumGenerator1: RandomNum { // 定义一个类遵守 RandomNum 协议
    func random() -> Int {
...
protocol BabyDelegate {
    func feedBaby(baby: Baby) // 喂食物的代理方法
}
...
class Nanny: BabyDelegate { // nanny 遵守代理
    func feedBaby(baby: Baby) { // nanny 实现喂食物的代理方法
        baby.needNumFood = 10
...
// 协议本身也是种类型可以放到集合中使用
let things: [SomeProtocol6] = [lucyTheCat] // 存放遵守 SomeProtocol6 协议的类
...
// 协议合成: 同时采纳多个协议, 多个协议之间用 '&' 分隔, 协议合成不会生成新的协议, 只是一个临时局部的
protocol Name {
    var name: String { get }
}
protocol Age {
...
say(to: aPeople) // 只要遵守这两个协议的对象都能传进去作为参数

// 检查协议的一致性, 如果不一致可以进行转换
// 'is' 检查实例是否符合某个协议, 符合返回 true, 否则返回 false
// 'as?' 如果符合某个协议类型, 则返回类型为该协议类型的可选值, 否则返回 nil
...
for object in objects {
    if let objectWithArea = object as? HasArea { // 判断 object 是否遵守 area 协议
        ...
        //print(objectWithArea.pi) // ❌ 编译报错: 所以只有 area 属性才能被访问
    } else {
        print("没有遵守 area 协议")
    }
}

// 协议的可选性: 协议中的所有方法和属性并不是一定都要实现的, 可以在可实现可不实现的前面加 optional 关键字, 使用可选方法或属性时, 它们的类型会自动变为可选的
// 注意: 可选的协议前面需要加 @objc 关键字
// @objc 表示该协议暴露给 ObjC 代码, 但即使不与 ObjC 交互只想实现可选协议, 还是要加 @objc 关键字
...
extension RandomNumG {
    var description: String {
        return "extension RandomNumG"
    }
    func randomBool() -> Bool { // 可以通过扩展为协议添加方法
