...
struct Weather {
    var temperature: Double           // 也可以直接给属性设置默认值
    init() {                          // 不带参数的构造方法
        temperature = 26.0            // 通常在构造器中给属性赋初值
    }
}
var aWeather = Weather()              // 初始化的过程会调构造方法
...
struct Point {
    ...
    // fromPointX 外部名称
    // '_' 省略外部名称
    init(fromPointX pointX: Double, _ pointY: Double) { // 自定义构造函数
        x += pointX
        y += pointY
        z = pointX // 常量在赋值后不可再更改
...
// 必要构造器: 子类必须实现的构造器
class SomeClass {
    required init() {          // 添加 required 修饰符表示该类的子类都必须实现该构造器
        // ...
    }
}
