...
// '~': 按位取反
let bits: UInt8 = 0b0011 // UInt8 的范围为: 0~255, bits 表示十进制的 3
let invertBits = ~bits // 按位取反运算符, 全部位取反, 得到十进制的 252

let firstTwoBits = 0b10 // 十进制的 2
let secondTwoBits = 0b11 // 十进制的 3

// '&': 按位与
let fourBits = firstTwoBits & secondTwoBits // 0b10

// '|': 按位或
let combinedBits = firstTwoBits | secondTwoBits // 0b11

// '^': 按位异或
let outputBits = firstTwoBits ^ secondTwoBits // 0b01

let shiftBits: UInt = 0b00110 // 十进制的 6

// '<<': 按位左移
shiftBits << 1 // 0b01100, 左移动一位, 十进制的 12

// '>>': 按位右移
shiftBits >> 1 // 0b00011, 右移动一位, 十进制的 3


/// 溢出运算符

var overflowInt: UInt8 = 255 // UInt8 的范围为: 0~255
//overflowInt = 256 // ❌ 数值超过了变量的存储范围编译器会报错
//overflowInt = -1  // ❌

// 数值溢出时可以做截断处理, 有以下几种方法可以处理方式:
// 1. &+ 溢出加法
// 2. &- 溢出减法
// 3. &* 溢出乘法
overflowInt = UInt8.max // 最大值 255
overflowInt = overflowInt &+ 1 // 二进制加1的时候发生溢出变成了 0
print(overflowInt)
overflowInt = UInt8.min // 最小值 0
overflowInt = overflowInt &- 1 // 二进制减1的时候发生溢出变成了 255
print(overflowInt)
